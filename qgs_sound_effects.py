...
from qgis.PyQt.QtCore import (
    ...
    QDateTime, QUrl, QMetaMethod
)
from qgis.PyQt.QtGui import QIcon
from qgis.PyQt.QtWidgets import (
    QAction, QCheckBox, QToolButton,
...
from .qgs_sound_effects_dialog import (
    QgisSoundEffectsDialog,
    QgisSoundEffectsConfigDialog
)
import os.path
...
class QgisSoundEffects:
        ...
        for sound in self.sounds_config.keys():
            ...
            effect.setSource(
                QUrl.fromLocalFile(
                    os.path.join(
                        self.plugin_dir,
                        self.sounds_config[sound]['filename']
                        )
                    )
                )
        ...
        if self.enabled is True:
            ...
            if enabled is True:
                self.iface.mapCanvas().layersChanged.connect(
                    self.bound_sounds['layersChanged'].play
                    )
            else:
                try:
            ...
            if enabled is True:
                self.iface.mapCanvas().renderComplete.connect(
                    self.bound_sounds['renderComplete'].play
                    )
            else:
                try:
            ...
            if enabled is True:
                self.iface.mapCanvas().renderErrorOccurred.connect(
                    self.bound_sounds['renderErrorOccurred'].play)
            else:
                try:
                    self.iface.mapCanvas().renderErrorOccurred.disconnect(
                        self.bound_sounds['renderErrorOccurred'].play)
                except Exception as e:  # noqa: F841
                    # This will happen if the event was not connected
                    pass
    ...
    def toggle_export_events(self):
        try:
            if self.enabled is True:
                ...
                if enabled is True:
                    self.iface.layoutDesignerOpened.connect(
                        self.attachLayoutDesignerExportSuccessListener
                        )
                else:
                    self.iface.layoutDesignerOpened.connect(
                        self.disconnectLayoutDesignerExportSuccessListener
                        )
                ...
                enabled = config.get('enabled', False)
                if enabled is True:
                    self.taskManager.taskAdded.connect(
                        self.mapExportSuccessTaskListener
                        )
                else:
                    try:
                        self.taskManager.taskAdded.disconnect(
                            self.mapExportSuccessTaskListener
                            )
                    except Exception as e:  # noqa: F841
                        # This will happen if the event was not connected
                ...
                if enabled is True:
                    self.taskManager.taskAdded.connect(
                        self.mapExportErrorTaskListener
                        )
                else:
                    try:
                        self.taskManager.taskAdded.disconnect(
            ...
            QgsMessageLog.logMessage(
                'Error toggling export events: {}'.format(e),
                MESSAGE_CATEGORY,
                Qgis.Critical
                )
            QgsMessageLog.logMessage(
    ...
    def attachLayoutDesignerExportSuccessListener(self, designer):
        designer.layoutExported.connect(
            self.bound_sounds['printLayoutExportSuccess'].play
            )

    def disconnectLayoutDesignerExportSuccessListener(self, designer):
        try:
            designer.layoutExported.disconnect(
                self.bound_sounds['printLayoutExportSuccess'].play
                )
        except Exception as e:  # noqa: F841
            # This will happen if the event was not connected
    ...
    def mapExportSuccessTaskListener(self, taskId):
        ...
        if (self.iface.tr("Saving as image") == task.description() or
                self.iface.tr("Saving as PDF") == task.description()):
            ...
            for i in range(task.metaObject().methodCount()):
                ...
                if method.methodType() == QMetaMethod.Signal:
                    if method.name() == 'renderingComplete':
                        task.renderingComplete.connect(
                            self.bound_sounds['mapExportComplete'].play
                            )
        else:
            pass
                ...
                if method.methodType() == QMetaMethod.Signal:
                    if method.name() == 'errorOccurred':
                        task.errorOccurred.connect(
                            self.bound_sounds['mapExportError'].play
                            )
        else:
            pass
    ...
    def check_processing_entry(self):
        ...
        try:
            ...
            processingSuccessEnabled = processingSuccessConfig.get(
                'enabled', False)
            processingFailureEnabled = processingFailureConfig.get(
                'enabled', False)

            # Check only the last second
            entries = self.history.queryEntries(
    ...
    def initGui(self):
        ...
        icon_path = os.path.join(self.plugin_dir,
                                 'qgs_effects_icon.png')
        config_icon_path = os.path.join(self.plugin_dir,
                                        'qgs_effects_config_icon.png')
        ...
        if type(sound_effects_enabled) is str:
            if sound_effects_enabled.lower() == 'true':
                ...
                self.enabled = False
        self.sound_effects_toggle.stateChanged.connect(
    ...
    def configure_settings_window(self):
        self.checkbox_ids = [
            'processingSuccessCheckBox', 'processingFailureCheckBox',
            'zoomInCheckBox', 'zoomOutCheckBox',
            'layersChangedCheckBox', 'renderCompleteCheckBox',
            'renderErrorOccurredCheckBox', 'mapExportCompleteCheckBox',
            'mapExportErrorCheckBox', 'printLayoutExportSuccessCheckBox'
            ]
        self.combobox_ids = [
            'processingSuccessComboBox', 'processingFailureComboBox',
            'zoomInComboBox', 'zoomOutComboBox',
            'layersChangedComboBox', 'renderCompleteComboBox',
            'renderErrorOccurredComboBox', 'mapExportCompleteComboBox',
            'mapExportErrorComboBox', 'printLayoutExportSuccessComboBox'
            ]
        self.test_button_ids = [
            'processingSuccessTestButton', 'processingFailureTestButton',
            'zoomInTestButton', 'zoomOutTestButton',
            'layersChangedTestButton', 'renderCompleteTestButton',
            'renderErrorOccurredTestButton', 'mapExportCompleteTestButton',
            'mapExportErrorTestButton', 'printLayoutExportSuccessTestButton'
            ]
        self.volume_ids = [
            'processingSuccessVolume', 'processingFailureVolume',
            'zoomInVolume', 'zoomOutVolume',
            'layersChangedVolume', 'renderCompleteVolume',
            'renderErrorOccurredVolume', 'mapExportCompleteVolume',
            'mapExportErrorVolume', 'printLayoutExportSuccessVolume'
            ]
        self.loop_ids = [
            'processingSuccessLoopSpinbox', 'processingFailureLoopSpinbox',
            'zoomInLoopSpinbox', 'zoomOutLoopSpinbox',
            'layersChangedLoopSpinbox', 'renderCompleteLoopSpinbox',
            'renderErrorOccurredLoopSpinbox', 'mapExportCompleteLoopSpinbox',
            'mapExportErrorLoopSpinbox', 'printLayoutExportSuccessLoopSpinbox'
            ]
        ...
        for i in range(0, len(self.checkbox_ids)):
            try:
                ...
                checkbox.stateChanged.connect(
                    self.make_toggle_event(checkbox_id)
                    )
                ...
                comboBox.currentIndexChanged.connect(
                    self.make_set_event_sound(combobox_id)
                    )

                test_button_id = self.test_button_ids[i]
                test_button = self.config_window.findChild(
                    QToolButton, test_button_id
                    )
                test_button.clicked.connect(self.make_test_sound(combobox_id))

                volume_id = self.volume_ids[i]
                volume = self.config_window.findChild(
                    QDoubleSpinBox, volume_id
                    )
                ...
                QgsMessageLog.logMessage(
                    'Error configuring settings window: {}'.format(str(e)),
                    MESSAGE_CATEGORY,
                    Qgis.Critical
                    )
                QgsMessageLog.logMessage(
        ...
        self.config_window.saveSettingsButton.clicked.connect(
            self.save_settings
            )
        self.config_window.cancelChangesButton.clicked.connect(
    ...
    def objectid_to_eventid(objid):
        objid = objid.replace('CheckBox', '')
        objid = objid.replace('ComboBox', '')
        objid = objid.replace('TestButton', '')
        objid = objid.replace('Volume', '')
        return objid
    ...
    def restore_settings(self):
        try:
            ...
            default_config = '''{\
                "layersChanged": {"enabled": false, "sound": "woodenfrog",\
                "sound_index": 10, "volume": 1.0, "loops": 0}, \
                "processingFailure": {"enabled": true, "sound": "fail", \
                "sound_index": 0, "volume": 1.0, "loops": 0}, \
                "processingSuccess": {"enabled": true, "sound": "success", \
                "sound_index": 1, "volume": 1.0, "loops": 0}, \
                "renderComplete": {"enabled": false, "sound": "fail", \
                "sound_index": 0, "volume": 1.0, "loops": 0}, \
                "renderErrorOccurred": {"enabled": false, "sound": "fail", \
                "sound_index": 0, "volume": 1.0, "loops": 0}, \
                "zoomIn": {"enabled": false, "sound": "synth-glide", \
                "sound_index": 6, "volume": 1.0, "loops": 0}, \
                "zoomOut": {"enabled": false, "sound": "synth-glide", \
                "sound_index": 6, "volume": 1.0, "loops": 0}, \
                "mapExportComplete": {"enabled": false, \
                "sound": "good_answer_harp", \
                "sound_index": 12, "volume": 1.0, "loops": 0}, \
                "mapExportError": {"enabled": false, "sound": "sad_trombone", \
                "sound_index": 14, "volume": 1.0, "loops": 0}, \
                "printLayoutExportSuccess": {"enabled": false, \
                "sound": "tada", "sound_index": 13, "volume": 1.0 }}
                '''
            config = json.loads(self.get_setting('config', default_config))
            return config
        except Exception as e:
            QgsMessageLog.logMessage(
                'Error restoring settings: {}'.format(str(e)),
                MESSAGE_CATEGORY,
                Qgis.Critical
                )
            return {}
    ...
    def save_settings(self):
        try:
            for i in range(0, len(self.checkbox_ids)):
                ...
                checkbox_id = self.checkbox_ids[i]
                checkbox = self.config_window.findChild(
                    QCheckBox,
                    checkbox_id
                    )
                ...
                combobox_id = self.combobox_ids[i]
                comboBox = self.config_window.findChild(
                    QComboBox,
                    combobox_id
                    )
                ...
                loop_id = self.loop_ids[i]
                loop = self.config_window.findChild(
                    QSpinBox,
                    loop_id
                    )
            ...
            QgsMessageLog.logMessage(
                'Error saving settings: {}'.format(str(e)),
                MESSAGE_CATEGORY,
                Qgis.Critical
                )
    ...
    def run(self):
        ...
        if self.first_start is True:
            ...
            # No dialog yet,
            # should be used for configuration of sounds to events
